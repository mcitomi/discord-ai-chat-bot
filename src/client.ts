    ...
    try {
        ...
        client.on("messageCreate", async (msg) => {
            ...
            if (msg.content.includes(`<@${client.user.id}>`)) {
                ...
                console.log(`user: ${msg.author.displayName} ; text: ${message}`);

                const chat = ai.chats.create({ model: 'gemini-2.0-flash', history });
                const response = await chat.sendMessage({
                    message: `user: ${msg.author.displayName} text: ${message}`,
                    config: {
                ...
                history.push({
                    ...
                    "parts": [
                        {
                            "text": `user: ${msg.author.displayName} ; text: ${message}`
                        }
                    ]
                },
                {
                    ...
                    "parts": [
                        {
                            "text": `user: ${client.user.displayName} ; text: ${response.text}`
                        }
                    ]
                })
            }
